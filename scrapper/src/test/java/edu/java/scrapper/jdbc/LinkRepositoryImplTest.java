...
@SpringBootTest class LinkRepositoryImplTest implements IntegrationTest {
    ...
    @Test @Transactional @Rollback void remove() {
        linkRepository.add(link);
        assertEquals(1, linkRepository.findAll().size());
        DTOLink curLink = new DTOLink(linkRepository.findByUrl(link.url()).linkId(), null, null, null);
    ...
    @Test @Transactional @Rollback void findAll() {
        ...
        assertEquals(1, linkRepository.findAll().size());
        assertEquals(
            "[DTOLink[linkId=" + linkRepository.findByUrl(link.url()).linkId() + ", url=https://test, updateAt=2024-03-17T21:31Z, checkAt=2024-03-17T22:11Z]]",
